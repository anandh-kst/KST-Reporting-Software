...
const AttendanceReport = () => {
  ...
  const fetchPunchData = async (selectedYear, selectedFromMonth, selectedToMonth) => {
    try {
      const startDate = moment(`${selectedYear}-${selectedFromMonth}-01`).startOf("month").format("YYYY-MM-DD");
      const endDate = moment(`${selectedYear}-${selectedToMonth}-01`).endOf("month").format("YYYY-MM-DD");
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/getPunchDataRange`, { 
        startDate, 
        endDate 
      });
      ...
      formattedData.forEach((punch) => {
        ...
        if (!punchMap[punch.employeeName].includes(punchDate)) {
  ...
  const getLeaveSymbol = (date, employeeLeaves) => {
    ...
    const holiday = holidays.find((h) => 
      (h.isActive === "1" && moment(h.eventStartDate).startOf("day").isSame(moment(date).startOf("day"))) || 
      moment(date).isBetween(moment(h.eventStartDate).startOf("day"), moment(h.eventEndDate).endOf("day"), "day", "[]")
    );
    ...
    employeeLeaves?.forEach((leave) => {
      if (moment(date).isBetween(leave.startDate, leave.endDate, "day", "[]")) {
        ...
        if (leave.leaveTypes.includes("Casual Leave")) leaveSymbol = "CL";
