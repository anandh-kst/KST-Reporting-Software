    ...
    <div>
      ...
      <div className="py-3">
        <input
          ...
          id={id}
          value={value}
          className="w-full sm:w-28 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5"
          placeholder=""
        />
      </div>
    </div>
...
const IdCardReport = () => {
    ...
    const fetchApplications = async () => {
      try {
        const response = await axios.post(
          `${process.env.REACT_APP_API_URL}/appId`
        );
  ...
  useEffect(() => {
    const fetchIdReportDetails = async () => {
      if (selectedIdCardId && employeeId) {
        ...
        try {
          const response = await axios.post(
            `${process.env.REACT_APP_API_URL}/getIdReportById/${employeeId}/${selectedIdCardId}`
          ); // Fetching using employeeId and reportId
          const { status, data } = response.data;
          if (status === "Success") {
            if (data.length > 0) {
  ...
  const handleInputChange = async (e) => {
    ...
    if (id === "application") {
      try {
        const locationIdresponse = await axios.post(
          `${process.env.REACT_APP_API_URL}/locationId/${value}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL}/post_id_report`,
        formData
      );
    ...
    <div className="pt-8 p-5 bg-gray-200 rounded-md">
      <form onSubmit={handleSubmit}>
        ...
        <div className="my-2 whitespace-nowrap grid grid-cols-2 xl:grid-cols-7 md:grid-cols-4 sm:grid-cols-2">
          {formTwo.map((label) => (
            <NumberInputField
              ...
              label={label}
              handleInputChange={handleInputChange}
              value={formData[toCamelCase(label)]}
            />
          ))}
        </div>
        <hr />
        ...
        <div className="flex items-center justify-between">
          <button
            type="submit"
            className="text-white bg-indigo-900 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center"
          >
            Submit
          </button>
        </div>
